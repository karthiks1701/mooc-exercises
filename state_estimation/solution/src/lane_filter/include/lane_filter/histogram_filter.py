...
def histogram_predict(belief, dt, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
        ...
        d_left = left_encoder_ticks / 656
        d_right = right_encoder_ticks / 656
        
        dist = (d_right + d_left)/2
        angle = (d_right - d_left)/(robot_spec["wheel_baseline"])
        ...
        d_t = grid_spec['d'] + dist*np.sin(angle) # replace this with something that adds the new odometry
        phi_t = grid_spec['phi'] + angle # replace this with something that adds the new odometry
